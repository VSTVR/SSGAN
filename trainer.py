...
class Trainer(object):
    ...
    def run_single_step(self, batch, batch_unlabel, step=None, is_train=True):
        ...
        fetch_values = self.session.run(fetch,
            feed_dict = self.model.get_feed_dict_withunlabel(batch_chunk, batch_chunk_unlabel, step=step)
        )

        [step, accuracy, d_loss, g_loss, \
         s_loss, all_preds, all_targets, g_img] = fetch_values[:8]

        _end_time = time.time()

        return step, accuracy, d_loss, g_loss, s_loss, \
            (_end_time - _start_time), all_preds, all_targets, g_img

    def run_test(self, batch, batch_unlabel, is_train=False):
        _start_time = time.time()

        batch_chunk = self.session.run(batch)
        batch_chunk_unlabel = self.session.run(batch_unlabel)

        [accuracy, summary, d_loss, g_loss, s_loss] = self.session.run(
            [self.model.accuracy, self.summary_op,  self.model.d_loss, 
             self.model.g_loss, self.model.S_loss],
            feed_dict=self.model.get_feed_dict_withunlabel(batch_chunk, batch_chunk_unlabel, is_training=False))

        _end_time = time.time()

        return accuracy, summary, d_loss, g_loss, s_loss, (_end_time - _start_time)
